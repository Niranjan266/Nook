...
REM ------------------------------------------------------------- Dependencies
if not exist "server\node_modules" (
  echo   [..] Installing server dependencies - a minute the first time
  pushd server
...
if !READY!==0 (
  echo.
  echo   [X] The API did not answer within 90 seconds.
...
echo     Database local file - server\data\nook.db
echo.
...
echo   ==========================================
echo.
echo   Two windows opened behind this one.
echo   Closing them stops Nook, or run Stop-Nook.bat
echo.
